...
public class WordBox {
  ...
  public void showTyped(String input) {
    // Label text = (Label) wordBox.getChildren().get(1); // Get the Label node from the StackPane
    // StringBuilder typedText = new StringBuilder();
    // for (int i = 0; i < input.length(); i++) {
    //   if (i < text.getText().length() && input.charAt(i) == text.getText().charAt(i)) {
    //     // typedText.append("<span style='color: purple;'>").append(input.charAt(i)).append("</span>");
    //     // Color.color(0.3, 0.3, 0.8); <- the color we want letters to change to when typed
    //     int r = (int) (0.3 * 255);
    //     int g = (int) (0.3 * 255);
    //     int b = (int) (0.8 * 255);
    //     // typedText.append("\033[38;2;" + r + ";" + "g" + ";" + "b" + "m");   // should hopefully make the purple we want
